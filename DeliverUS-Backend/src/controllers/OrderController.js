...
const create = async (req, res) => {
  ...
  try {
    ...
    for (const product of req.body.products) {
      const productSelect = await Product.findByPk(product.productId)
      newOrder.price += productSelect.price * product.quantity
    }
    ...
    for (const product of req.body.products) {
      const productSelect = await Product.findByPk(product.productId)
      await newOrder.addProduct(productSelect, { through: { quantity: product.quantity, unityPrice: productSelect.price }, transaction })// Cambiar por propiedades de base de datos
    }

    await transaction.commit()
    const orderRes = await Order.findByPk(newOrder.id, { include: [{ model: Product, as: 'products' }] })
    res.json(orderRes)
  } catch (err) {
...
const update = async function (req, res) {
  ...
  try {
    ...
    let orderPrice = req.body.price || 0
    let orderShippingCosts = rest.shippingCosts
    const orderAddress = req.body.address || order.address
    const orderRest = req.body.restaurantId || order.restaurantId

    for (const product of req.body.products) {
      const productSelect = await Product.findByPk(product.productId)
      orderPrice += productSelect.price * product.quantity
    }
    ...
    await Order.update({
      ...
      restaurantId: orderRest
    }, { where: { id: order.id }, transaction })
    await order.setProducts(null, { transaction })
    for (const product of req.body.products) {
      const productSelect = await Product.findByPk(product.productId)
      await order.addProduct(productSelect, { through: { quantity: product.quantity, unityPrice: productSelect.price }, transaction })// Cambiar por propiedades de base de datos
    }
    await transaction.commit()
...
const destroy = async function (req, res) {
  try {
    ...
    let message = ''
    if (result === 1) {
      message = 'Sucessfuly deleted order id' + req.params.productId
    } else {
  ...
  try {
    ...
    const invoicedToday = await Order.sum(
      'price',
      {
        where:
        {
          createdAt: { [Op.gte]: todayZeroHours }, // FIXME: Created or confirmed? Confirmed
          restaurantId: req.params.restaurantId
        }
      })
    res.json({
